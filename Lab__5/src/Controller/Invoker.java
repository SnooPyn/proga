...
/**
 * The Invoker class's an object that knows how the command has been implemented.
 * It only knows the Command's interface.
 * It calls the Command and says what needs to be done (each command is a class with logic).
 */
public class Invoker {
    /**
     * A map of commands, where the key is the string value (the name of the command),
     * and the value is the command object itself
     */
    ...
    * in the method argument we pass a string containing the command and its argument("update 1" or "show").
     * The most important method, it implements the logic of the command.
     */
            ...
            if (!commandName.equals("")) {
                ...
                if (commands.get(nameAndArgument[0]) == null) {
                    ...
                    if (commands.get(nameAndArgument[0]) instanceof CommandWithoutArg) {
                        ...
                        try {
                            /**
                             * We will try to get an argument from the array nameAndArgument
                             ...
                             * we get the command object by the name of the command and the executive without an argument
                             * (that is, we pass null in the arguments of the method)
                             */
                        ...
                        try {
                            ...
                            try {
                                // We get the command object by the name of the command and the executive with the argument nameAndArgument[1]
